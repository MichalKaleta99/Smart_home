...
import os
import time
import random # added to display random lighting values - no physical sensors needed
...
def digital_lighting_value():
      ...
      result=random.uniform(8.498,183.5)
      digital_lighting_percent_value = int(100* (min_digital_light-result)/(min_digital_light-max_digital_light))
...
def adc_lighting_value():
    ...
    measured_adc_light = random.uniform(5213,39714)
    measured_adc_light = int(measured_adc_light)
...
def capacitor_lighting_value():
    ...
    capacitor_charging_seconds = random.uniform(0.016769, 0.27627)
