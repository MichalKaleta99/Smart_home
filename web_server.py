...
def index_html():
    ...
    if request.method=='POST':
        ...
        if user:
            user_name = user[0]
            user_permissions = user[2]
            return redirect(url_for('home_html', user_permissions = user_permissions))
        else:  
...
def temp_html():
    if request.method == 'POST':
        if "fan_automatic" in request.form:  
            ...
            temp_control_stop()   
    templateData = {
        #'fan_fdbck_percentage' : fan_fdbck() ### read and display feedback value (% of RPM) from fan
        }   
...
def home_html():
    with sqlite3.connect("mydb.db") as con:
         global user_name
         c = con.cursor()        
         c.execute('SELECT date, temp FROM tmp order by date desc limit 24')
         data = c.fetchall()
         data = data[::-1]
         dates = []
         values = []
    ...
    return render_template('home.html', labels = labels, values = values, user_permissions = user_permissions, **templateData)
@app.route("/templates/watering/")
def watering():
     if request.method == 'POST':
        if "watering" in request.form:  
            humidity_sp = request.form['humidity_sp']
...
@app.route("/templates/settings/", methods=['POST', 'GET'])
def settings_html():
    if request.method == 'POST':
        if "change_user_password" in request.form:
            change_pass = True
            print('test')
